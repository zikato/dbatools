function Suspend-DbaDbEncryption {
    <#
    .SYNOPSIS
        Suspends decryption for one or more databases on an instance
    ...
    .PARAMETER Database
        The specific databases where encryption will be suspended

    .PARAMETER ExcludeDatabase
        The specific databases where encryption will not be suspended
    ...
    .LINK
        https://dbatools.io/Suspend-DbaDbEncryption

    .EXAMPLE
        PS C:\> Suspend-DbaDbEncryption -SqlInstance sql01

        Suspends decryption for all databases on a SQL Server instance

        Prompts for confirmation along the way

    .EXAMPLE
        PS C:\> Suspend-DbaDbEncryption -SqlInstance sql01 -Database db1 -Confirm:$false
    ...
    process {
        ...
        foreach ($db in $InputObject) {
            ...
            if ($db.Name -in 'master', 'model', 'tempdb', 'msdb', 'resource') {
                Write-Message -Level Verbose "Encryption was not suspended for $($db.Name) on $($server.Name) because this database cannot be encrypted"
                continue
            }
            $server = $db.Parent
            Write-ProgressHelper -StepNumber ($stepCounter++) -Message "Suspending encryption for $($db.Name) on $($server.Name)" -TotalSteps $InputObject.Count
            try {
                ...
                if (($db.DatabaseEncryptionKey.EncryptionState -in "EncryptionInProgress", "DecryptionInProgress", "EncryptionKeyChangesInProgress")) {
                    ...
                    $null = $db.Query("ALTER DATABASE [$($db.Name)] SET ENCRYPTION SUSPEND")
                    $null = $db.Refresh()
                    ...
                    Write-Message -Level Verbose "Encryption for $($db.Name) on $($server.Name) has been suspended"
                    ...
                    Write-Message -Level Verbose "Encryption for $($db.Name) on $($server.Name) is not in a state to be suspended"
                    Add-Member -Force -InputObject $db -MemberType NoteProperty -Name EncryptionState -value $db.DatabaseEncryptionKey.EncryptionState
                    $db | Select-DefaultView -Property ComputerName, InstanceName, SqlInstance, 'Name as DatabaseName', EncryptionEnabled, EncryptionState
                }
            } catch {
